...
def creditos():
    ...
    while not sair:
        ...
        surface_texto, rect_texto = text_objects("Grupo 2", fonte, BRANCO)
        rect_texto.center = ((LARGURA / 2), ALTURA / 3)
        display.blit(surface_texto, rect_texto)

        surface_texto, rect_texto = text_objects("Disciplina: Projeto Integrador em Computação I", fonte,
                                                 BRANCO)
...
def regras():
    ...
    while not sair:
        ...
        info1 = fonte.render('O jogo de damas eh praticado em um tabuleiro de 64 casas.', False, (BRANCO))
        info2 = fonte.render('O objetivo do jogo eh capturar todas as pecas do oponente.', False, (BRANCO))
        info3 = fonte.render('A peca anda soh para frente, uma casa de cada vez, na diagonal.', False, (BRANCO))
        info4 = fonte.render('Quando a peca atinge a oitava linha do tabuleiro ela vira dama.', False, (BRANCO))
        info5 = fonte.render('A dama eh uma peca de movimentos mais amplos. Ela anda para frente e para tras,', False,
                             (BRANCO))
        info6 = fonte.render('quantas casas quiser, nao podendo saltar sobre uma peca da mesma cor. ', False, (BRANCO))
        info7 = fonte.render('A captura e obrigatoria, ou seja, nao existe sopro.', False, (BRANCO))
        info8 = fonte.render('Duas ou mais pecas juntas, na mesma diagonal, nao podem ser capturadas.', False,
                             (BRANCO))
        info9 = fonte.render('A peca e a dama podem capturar tanto para frente como para tras.', False, (BRANCO))
        info10 = fonte.render('O movimento de captura pode ser encadeado sem que o jogador passe a vez.', False, (BRANCO))
